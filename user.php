<?php
require_once('includes/init.php'); // Pastikan file ini ada dan berisi koneksi database
$user_role = get_role(); // Asumsi fungsi get_role() ada dan mengembalikan peran user yang login
...
<head>
    ...
    <style>
        ...
        .sidebar {
            width: 250px; background-color:rgb(39, 41, 33);
        ...
        .sidebar .nav-link:hover, .sidebar .nav-link.active {
            background-color:rgb(5, 108, 212); color: #fff;
        }
        .sidebar .nav-link i {
            font-size: 1.1rem;
        }
        ...
        .sidebar a:hover {
            background-color: #495057; text-decoration: none;
        }
        .sidebar a i {
            margin-right: 10px; font-size: 1.2rem;
        }
        .content {
            margin-left: 250px; flex-grow: 1;
        ...
        .top-navbar {
            ...
            align-items: center; box-shadow: 0 2px 4px rgba(172, 10, 10, 0.1);
        }
        .menu-toggle {
...
<div id="content" class="content">
    <div class="top-navbar">
        ...
        <div class="dropdown">
            <a class="d-flex align-items-center text-dark text-decoration-none dropdown-toggle" href="#" id="dropdownUser" data-bs-toggle="dropdown" aria-expanded="false">
                <span class="me-2 text-uppercase fw-normal"><?php echo htmlspecialchars($current_username); ?></span>
        ...
        <div class="d-flex align-items-center gap-2 mb-3">
            <form class="d-flex" method="GET" action="">
                <div class="input-group">
                    <input type="search" name="keyword" class="form-control shadow-sm" placeholder="Cari nama user..." value="<?= isset($_GET['keyword']) ? htmlspecialchars($_GET['keyword']) : '' ?>">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            if (!empty($keyword)) {
                                $query_sql .= " WHERE nama LIKE '%" . $koneksi->real_escape_string($keyword) . "%' OR username LIKE '%" . $koneksi->real_escape_string($keyword) . "%'";
                            }
                            ...
                            while($row = mysqli_fetch_assoc($query)) {
                                ...
                                echo "<td>
                                    <button type='button' class='btn btn-warning btn-sm me-1 btn-edit-user'
                                    ...
                                    <a href='hapus_user.php?id={$row['id_admin']}' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin ingin menghapus user ini?\")'>
    ...
    <div class="modal fade" id="modalEditUser" tabindex="-1" aria-labelledby="modalEditUserLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-md">
            <form action="up/edit-user.php" method="post" class="modal-content rounded-3">
...
document.addEventListener('DOMContentLoaded', function () {
    var editUserModal = document.getElementById('modalEditUser');
    editUserModal.addEventListener('show.bs.modal', function (event) {
        ...
        var modalIdAdmin = editUserModal.querySelector('#edit_id_admin');
        var modalNama = editUserModal.querySelector('#edit_nama');
        var modalUsername = editUserModal.querySelector('#edit_username');
        var modalPassword = editUserModal.querySelector('#edit_password'); // Reset password field

        modalIdAdmin.value = id_admin;
        modalNama.value = nama;
        modalUsername.value = username;
        modalPassword.value = ''; // Kosongkan password agar tidak terkirim hashed atau lama
    });
});
</script>
